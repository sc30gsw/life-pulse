...
- **W4 PR2 (相関ビュー+グローバルヘッダー)**: done. FR-7 `/insights` route (`convex/queries/insights/correlations.ts` join query + `convex/services/insights/pearson.ts` pure function; sleep/Body-Battery-vs-study scatter charts, HIIT Body Battery bar chart, daily aligned bar/line view, workout-kind polar view, all via TanStack Charts 0.7.2), plus global header promotion (`BoardHeader` lifted into the `_authenticated` layout, per-page 戻る links removed, UserMenu insights nav link for self).
