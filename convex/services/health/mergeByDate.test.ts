...
test("demoMode on: prefers demo, then garmin, then manual per date", () => {
  const rows = [
  ...
  const merged = mergeByDate(rows, true);

  expect(merged).toHaveLength(2);
  expect(merged[0]).toMatchObject({ dateJst: "2026-07-01", source: "demo", steps: 3000 });
  expect(merged[1]).toMatchObject({ dateJst: "2026-07-02", source: "garmin", steps: 5000 });
});

test("demoMode off: excludes demo rows and prefers garmin over manual", () => {
  const rows = [
    buildRow({ dateJst: "2026-07-01", source: "manual", steps: 1000 }),
    buildRow({ dateJst: "2026-07-01", source: "garmin", steps: 2000 }),
    buildRow({ dateJst: "2026-07-01", source: "demo", steps: 3000 }),
  ...
  const merged = mergeByDate(rows, false);

  expect(merged).toHaveLength(2);
  expect(merged[0]).toMatchObject({ dateJst: "2026-07-01", source: "garmin", steps: 2000 });
  expect(merged[1]).toMatchObject({ dateJst: "2026-07-02", source: "manual", steps: 4000 });
});
...
test("returns exactly one row per distinct date, sorted ascending by dateJst", () => {
  ...
  expect(mergeByDate(rows, false).map((row) => row.dateJst)).toEqual([
    "2026-07-01",
    "2026-07-02",
    "2026-07-03",
  ]);
...
test("preserves the chosen row's original fields", () => {
  ...
  expect(mergeByDate(rows, false)[0]).toMatchObject({ bodyBattery: 80, hrv: 55, sleepScore: 70 });
});

test("supports the dashboard.health single-date composition: mergeByDate(rows, demoMode)[0] ?? null", () => {
  const rows = [
    buildRow({ dateJst: "2026-07-07", source: "manual", steps: 500 }),
    buildRow({ dateJst: "2026-07-07", source: "garmin", steps: 900 }),
  ];

  expect(mergeByDate(rows, false)[0] ?? null).toMatchObject({ source: "garmin", steps: 900 });
  expect(mergeByDate([], false)[0] ?? null).toBeNull();
});
