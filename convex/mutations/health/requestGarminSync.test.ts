...
test("schedules a 28-day backfill before Garmin metrics exist", async () => {
...
test("schedules a 28-day backfill when only two Garmin metric days exist", async () => {
  const t = convexTest(schema, testModules);
  const asSelf = t.withIdentity({ subject: "user_1" });
...
test("schedules the daily two-day sync after 28 Garmin metric days exist", async () => {
  const t = convexTest(schema, testModules);
  const asSelf = t.withIdentity({ subject: "user_1" });
  await t.run(async (ctx) => {
    await ctx.db.insert("appUsers", { authSubject: "user_1", displayName: "本人", role: "self" });
    for (let day = 1; day <= 28; day += 1) {
      await ctx.db.insert("healthMetrics", {
        dateJst: `2026-07-${String(day).padStart(2, "0")}`,
        source: "garmin",
        syncedAt: 1000,
      });
    }
