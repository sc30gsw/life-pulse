...
import { LiveStripView } from "~/features/dashboard/components/live-strip";
import { renderWithMantine } from "~/test-utils";

test("renders the last-sync relative label", () => {
  const { getByText } = renderWithMantine(<LiveStripView lastSyncRelativeLabel="5分前" />);

  expect(getByText("5分前")).toBeDefined();
});

test("renders the static Convex live-sync and 2-device labels", () => {
  const { getByText } = renderWithMantine(<LiveStripView lastSyncRelativeLabel="たった今" />);
