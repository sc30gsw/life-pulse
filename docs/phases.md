...
- [x] FR-3.6 未来日の枠宣言(DateTimePicker range、同日+過去日拒否)、予定一覧(今日超〜30日)、planned 枠の編集/物理キャンセル、日本の祝日表示
...
- [ ] FR-4.1 「断食開始」で eating → fasting。`targetMinutes` は開始時に任意指定、省略時は `appSettings.fastingDefaultMinutes`(既定 960 分)(v1.2 補足)
- [ ] FR-4.2 フェーズ遷移は scheduled function がサーバ側で書き換え(ポーリング禁止)。AC-2 のデモ短縮は開始時の `targetMinutes` 指定だけで実現(専用の隠し設定画面は不要、v1.2 補足)
- [ ] FR-4.3 「食事開始」で実績確定+予約済み未来遷移ジョブのキャンセル。断食中でない状態での終了操作は無視/エラー(v1.2 補足、FR-2 の二重操作ガードと同方針)
- [ ] FR-4.4 ライブボード+セッション画面に経過・現フェーズ・残りを常時表示(FR-1.2 の一部を解消)

### FR-4 UI 配線(PR1)

- [ ] ライブボードの断食操作ボタン配線(開始/終了)
