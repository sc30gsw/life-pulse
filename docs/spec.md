...
| health        | requestGarminSync                 | m              | self専用手動同期トリガー。`requireSelf` → `ctx.scheduler.runAfter(0, internal.actions.garmin.syncDaily.syncDaily)` を予約(CVX-05/17)。結果はsyncLogsのリアクティブ購読でUIに自動反映される                                                                                                                                                                  |
...
- 手動実行: `/health`の「今すぐ同期」ボタンから`requestGarminSync`(public mutation、`requireSelf`)を呼び、`ctx.scheduler.runAfter(0, internal.actions.garmin.syncDaily.syncDaily)`を即時予約する(CVX-05/17)。結果は`syncLogs`のリアクティブ購読で自動的にUIへ反映される。
