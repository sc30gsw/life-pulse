...
| health        | requestGarminSync                 | m              | self専用手動同期トリガー。`requireSelf` → Garminの成功ログがなければ `ctx.scheduler.runAfter(0, internal.actions.garmin.syncDaily.backfill)`、成功ログがあれば `syncDaily` を予約(CVX-05/17)。結果はsyncLogsのリアクティブ購読でUIに自動反映される                                                                                                          |
...
- 手動実行: `/health`の「今すぐ同期」ボタンから`requestGarminSync`(public mutation、`requireSelf`)を呼び、初回は`ctx.scheduler.runAfter(0, internal.actions.garmin.syncDaily.backfill)`で直近28日分、2回目以降は`syncDaily`で前日+当日を即時予約する(CVX-05/17)。結果は`syncLogs`のリアクティブ購読で自動的にUIへ反映される。
